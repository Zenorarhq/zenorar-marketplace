  ...
  async purchase(params: {
    ...
    try {
      const body: Record<string, unknown> = {
        ...
        fields: [
          { key: 'recipient.msisdn', value: params.recipientPhone.replace(/^\+/, '') },
          { key: 'sender.msisdn', value: (params.senderPhone || params.recipientPhone).replace(/^\+/, '') },
        ],
      }

      // For range-priced offers, include value as dto.PurchaseValue { type, value }
      if (params.value !== undefined) {
        body.value = { type: 'PRICE', value: Math.round(params.value * 100) }
      }

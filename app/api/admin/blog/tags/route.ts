...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[AdminBlog] Tags list error:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await executeQuery(
      'INSERT INTO blog_tags (name, slug) VALUES ($1, $2) ON CONFLICT (slug) DO NOTHING RETURNING *',
      [name, slug]
    )

    if (result.rows.length === 0) {
      const existing = await executeQuery('SELECT * FROM blog_tags WHERE slug = $1', [slug])
      return NextResponse.json({ success: true, data: existing.rows[0] })
    }

    return NextResponse.json({ success: true, data: result.rows[0] }, { status: 201 })
  } catch (error) {
    console.error('[AdminBlog] Tag create error:', error)
